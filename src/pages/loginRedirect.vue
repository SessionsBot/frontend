...
onMounted(() => {
    ...
    if (token) {
        // Token provided:
        statusFooterText.innerText = `token: ${token}`
        auth.loginWithToken(token)
        if(redirect) {router.push('/dashboard')}

    } else {
        // Token NOT provided:
        statusFooterText.innerText = `Login Failed!`
